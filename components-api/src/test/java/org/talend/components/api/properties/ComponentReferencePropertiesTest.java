...
public class ComponentReferencePropertiesTest {
    ...
    public void test() {
        ...
        fep.refAnother.getReference().strprop2.setValue("val2");
        fep.refAnother.componentInstanceId.setValue("12345");
        fep.refAnother.referenceType.setValue(ReferenceType.COMPONENT_INSTANCE);
        assertEquals("val1", fep.strprop.getValue());
        assertEquals("val2", fep.refAnother.getReference().strprop2.getValue());
        assertEquals("12345", fep.refAnother.componentInstanceId.getValue());
        assertEquals(ReferenceType.COMPONENT_INSTANCE, fep.refAnother.referenceType.getValue());
        ...
        assertEquals("val2", unserializedFep.refAnother.getReference().strprop2.getValue());
        assertEquals("12345", unserializedFep.refAnother.componentInstanceId.getValue());
        assertEquals(ReferenceType.COMPONENT_INSTANCE, fep.refAnother.referenceType.getValue());
    ...
    public void testUnserializeOldReferenceProps() {
        ...
        assertEquals("val2", unserializedFep.refAnother.getReference().strprop2.getValue());
        assertEquals("12345", unserializedFep.refAnother.componentInstanceId.getValue());
        assertEquals(ReferenceType.COMPONENT_INSTANCE, unserializedFep.refAnother.referenceType.getValue());
    }

    public static class PropWithReference extends ComponentPropertiesImpl implements ComponentReferencePropertiesEnclosing {

        Property<String> strprop = PropertyFactory.newString("strprop");

        public final ComponentReferenceProperties<AnotherProperties> refAnother = new ComponentReferenceProperties<>("refAnother",
                "AnotherPropDef");
        ...
        public void setupProperties() {
            ...
            refAnother.setReference(new AnotherProperties("componentProperties").init());
        }
        ...
        public void setupLayout() {
            ...
            Widget refWidget = Widget.widget(refAnother).setWidgetType(Widget.COMPONENT_REFERENCE_WIDGET_TYPE);
